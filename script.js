...
const messageFiles = [
    ...
    , '0089_28032026.html'
    , '0090_03042026.html'
    , '0091_05042026.html'    , '0092_11042026.html'];
...
const initMessageNavigator = () => {
    ...
    const closeAllYears = () => {
        yearBlocks.forEach((block) => {
            ...
            const heading = block.querySelector('.year-heading');
            if (content) {
    ...
    const showYear = (year) => {
        yearBlocks.forEach((block) => {
            ...
            if (heading) {
                heading.classList.toggle('open', matches);
            }
        });
    };
    ...
    yearBlocks.forEach((block) => {
        ...
        if (heading) {
            heading.addEventListener('click', () => toggleYear(block));
        }
    });

    const defaultYear = yearSelect.dataset.defaultYear || '';
